...
StaticObject::StaticObject(Game* game) : game(game){
...
StaticObject::StaticObject(Game* game, std::string name) : game(game){
	bodyDef.type = b2_staticBody;
	hasPhysics = true;
	objectType = STATIC;
	this->name = name;
...
bool StaticObject::loadFromFile(sf::Sprite* container, std::string filename){
...
bool StaticObject::loadFromFile(sf::CircleShape* container, std::string filename){
	if(!texture.loadFromFile(filename)){
		return false;
	}
	size = sf::Vector2f((float)texture.getSize().x, (float)texture.getSize().y);
...
bool StaticObject::loadFromFile(sf::ConvexShape* container, std::string filename){
	if(!texture.loadFromFile(filename)){
		return false;
	}
	size = sf::Vector2f((float)texture.getSize().x, (float)texture.getSize().y);
...
void StaticObject::setSize(sf::Vector2f size){
	if(hasImage){
